...
namespace DACN_QLCF
{
    public partial class ReportPKK : Form
    {
        ...
        private void Report_Load(object sender, EventArgs e)
        {
            var listPKK = (from s in data.PhieuKiemHangTons.ToList()
                          where s.NgayLapPhieu >= since.AddDays(-1)
                          && s.NgayLapPhieu <= to
                          select s).ToList();
            ...
            this.reportViewer1.LocalReport.ReportPath = @"../../TKPhieuKiemKho.rdlc";
            this.reportViewer1.LocalReport.SetParameters(param);
            var reportDataSource = new ReportDataSource("PhieuKiemKho", listPKK);
